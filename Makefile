...
REPOSITORY = docker-port-forward
...
Forward one or more local ports to a Docker container, with kubectl port-forward-style CLI and Docker Compose service resolution.
endef
...
test:
	go test -v ./internal/... ./portforward/...

test-integration:
	go test -v -tags=integration -count=1 -timeout=10m ./internal/...

test-bats: build/$(shell uname -s | tr A-Z a-z)/$(NAME)-$(shell uname -m | sed 's/x86_64/amd64/;s/aarch64/arm64/')
	bats test.bats

coverage:
	go test -coverprofile=coverage.out ./internal/... ./portforward/...
