...
5. Creates a container from the helper image with:
   - `--network <target-network>` and `-p <addr>:<local>:<remote>` for every requested port pair and each of its addresses.
...
Every helper records the network it shares with the target (`com.dokku.port-forward.target-network`) and what it dials there (`com.dokku.port-forward.target-address`): the target's container name on a user-defined network, or its IP on the default `bridge`. Tools such as dokku can compare these labels with the target's current state, or use `docker port-forward list --stale` (or `portforward.List` from Go) to find helpers that can no longer reach their target.

On the default `bridge`, a target that restarts can come back with a different IP, leaving its helper dialing the old one. Re-running the same `docker port-forward --detach` command replaces such a helper, and `docker port-forward cleanup --stale` removes every stale helper. See [`port-forward list`](command-reference.md#port-forward-list) for the exact rules.

## Logging

By default the helper uses the daemon's logging driver. Use `--log-driver` and `--log-opt`, which take the same values as `docker container create`, to change it:

```bash
docker port-forward --detach --log-driver json-file --log-opt max-size=10m my-container 8080:80
```

These settings apply to the helper only. The short-lived auto-detect probe keeps the daemon default, because the plugin reads the probe's output through its logs.
