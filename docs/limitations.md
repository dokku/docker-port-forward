...
- **Target IP changes on the default `bridge` network:** on a user-defined network the helper reaches the target by container name, so a target restart that changes its IP is handled. On the default `bridge` network there is no embedded DNS, so the helper uses the target's IP and a detached forward stops working if the target comes back with a different IP. Put the target on a user-defined network, or remove the helper and re-run the command.
- **Recreated targets:** if the target is recreated rather than restarted (for example `docker compose up` after a config change), the helper keeps forwarding to the new container by name. Its `com.dokku.port-forward.target` label still holds the old container ID, though, so `docker port-forward cleanup --target <new>` won't match it. Use `cleanup --name <helper>` or plain `cleanup` instead.
